...
describe('Environment Variables Validation', () => {
  ...
  it('should validate required environment variables', () => {
    ...
    vi.stubEnv('GITHUB_APP_PRIVATE_KEY_BASE64', Buffer.from('test-private-key').toString('base64'));
    vi.stubEnv('GITHUB_WEBHOOK_SECRET', 'test-webhook-secret');

    // 環境変数が正しく設定されていることを確認
    expect(process.env.GITHUB_APP_ID).toBe('12345');
    expect(process.env.GITHUB_APP_PRIVATE_KEY_BASE64).toBe(Buffer.from('test-private-key').toString('base64'));
  ...
  it('should create proper check run payload structure', () => {
    const checkRunPayload = {
      ...
      output: {
        ...
        summary: 'This is a simple \'Hello, world!\' message from your GitHub App for PR #123.',
